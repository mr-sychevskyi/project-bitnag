...
return s;})({1:[function(_dereq_,module,exports){(function(global){"use strict";_dereq_(327);_dereq_(328);_dereq_(2);if(global._babelPolyfill){throw new Error("only one instance of babel-polyfill is allowed");}
...
last=undefined;if(parent)parent.enter();};if(isNode){notify=function notify(){process.nextTick(flush);};}else if(Observer){var toggle=true;var node=document.createTextNode('');new Observer(flush).observe(node,{characterData:true});notify=function notify(){node.data=toggle=!toggle;};}else if(Promise&&Promise.resolve){var promise=Promise.resolve();notify=function notify(){promise.then(flush);};}else{notify=function notify(){macrotask.call(global,flush);};}
return function(fn){var task={fn:fn,next:undefined};if(last)last.next=task;if(!head){head=task;notify();}
last=task;};};},{"113":113,"18":18,"40":40}],69:[function(_dereq_,module,exports){'use strict';var aFunction=_dereq_(3);function PromiseCapability(C){var resolve,reject;this.promise=new C(function($$resolve,$$reject){if(resolve!==undefined||reject!==undefined)throw TypeError('Bad Promise constructor');resolve=$$resolve;reject=$$reject;});this.resolve=aFunction(resolve);this.reject=aFunction(reject);}
...
queue[++counter]=function(){invoke(typeof fn=='function'?fn:Function(fn),args);};defer(counter);return counter;};clearTask=function clearImmediate(id){delete queue[id];};if(_dereq_(18)(process)=='process'){defer=function defer(id){process.nextTick(ctx(run,id,1));};}else if(Dispatch&&Dispatch.now){defer=function defer(id){Dispatch.now(ctx(run,id,1));};}else if(MessageChannel){channel=new MessageChannel();port=channel.port2;channel.port1.onmessage=listener;defer=ctx(port.postMessage,port,1);}else if(global.addEventListener&&typeof postMessage=='function'&&!global.importScripts){defer=function defer(id){global.postMessage(id+'','*');};global.addEventListener('message',listener,false);}else if(ONREADYSTATECHANGE in cel('script')){defer=function defer(id){html.appendChild(cel('script'))[ONREADYSTATECHANGE]=function(){html.removeChild(this);run.call(id);};};}else{defer=function defer(id){setTimeout(ctx(run,id,1),0);};}}
...
while(x2>=2){n+=1;x2/=2;}
return n;};$export($export.P+$export.F*(!!$toFixed&&(0.00008.toFixed(3)!=='0.000'||0.9.toFixed(0)!=='1'||1.255.toFixed(2)!=='1.25'||1000000000000000128.0.toFixed(0)!=='1000000000000000128')||!_dereq_(35)(function(){$toFixed.call({});})),'Number',{toFixed:function toFixed(fractionDigits){var x=aNumberValue(this,ERROR);var f=toInteger(fractionDigits);var s='';var m=ZERO;var e,z,j,k;if(f<0||f>20)throw RangeError(ERROR);if(x!=x)return'NaN';if(x<=-1e21||x>=1e21)return String(x);if(x<0){s='-';x=-x;}
...
proto.constructor=$RegExp;$RegExp.prototype=proto;_dereq_(94)(global,'RegExp',$RegExp);}
...
return result;};if(!USE_NATIVE){$Symbol=function _Symbol2(){if(this instanceof $Symbol)throw TypeError('Symbol is not a constructor!');var tag=uid(arguments.length>0?arguments[0]:undefined);var $set=function $set(value){if(this===ObjectProto)$set.call(OPSymbols,value);if(has(this,HIDDEN)&&has(this[HIDDEN],tag))this[HIDDEN][tag]=false;setSymbolDesc(this,tag,createDesc(1,value));};if(DESCRIPTORS&&setter)setSymbolDesc(ObjectProto,tag,{configurable:true,set:$set});return wrap(tag);};redefine($Symbol[PROTOTYPE],'toString',function toString(){return this._k;});$GOPD.f=$getOwnPropertyDescriptor;$DP.f=$defineProperty;_dereq_(77).f=gOPNExt.f=$getOwnPropertyNames;_dereq_(82).f=$propertyIsEnumerable;_dereq_(78).f=$getOwnPropertySymbols;if(DESCRIPTORS&&!_dereq_(60)){redefine(ObjectProto,'propertyIsEnumerable',$propertyIsEnumerable,true);}
...
observer.complete();}});return function(){done=true;};});},of:function of(){for(var i=0,l=arguments.length,items=Array(l);i<l;){items[i]=arguments[i++];}
...
return ContinueSentinel;}};}(_typeof(global)==="object"?global:(typeof window==="undefined"?"undefined":_typeof(window))==="object"?window:(typeof self==="undefined"?"undefined":_typeof(self))==="object"?self:this);}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{});},{}]},{},[1]);

"use strict";$(function(){var form=$('.email-form');$(form).submit(function(e){e.preventDefault();var formData=$(form).serialize();$.ajax({type:'POST',url:$(form).attr('action'),data:formData,dataType:'json'}).done(function(response){$('.email-form__input').val('');}).fail(function(data){console.log('error');});});});

"use strict";var countdownTimerField=document.querySelector('.countdown-timer');var daysField=document.querySelector('.timer-days');var hoursField=document.querySelector('.timer-hours');var minutesField=document.querySelector('.timer-minutes');var secondsField=document.querySelector('.timer-seconds');var countDownDate=new Date("Dec 31, 2019 12:00:00").getTime();var countdownTimer=setInterval(function(){var now=new Date().getTime();var timeLeft=countDownDate-now;daysField.innerHTML=Math.floor(timeLeft/(1000*60*60*24));hoursField.innerHTML=Math.floor(timeLeft%(1000*60*60*24)/(1000*60*60));minutesField.innerHTML=Math.floor(timeLeft%(1000*60*60)/(1000*60));secondsField.innerHTML=Math.floor(timeLeft%(1000*60)/1000);if(timeLeft<0){clearInterval(countdownTimer);countdownTimerField.innerHTML='<span class="timer-notification">Expired</span>';}},1000);

"use strict";$('.header .ui.dropdown').dropdown();$('.footer .ui.dropdown').dropdown({direction:'upward'});
...
var placeholders=document.querySelectorAll('input[placeholder]');var placeholdersArr=_toConsumableArray(placeholders);var placeholdersFirst=_toConsumableArray(placeholders)[0];if(placeholdersArr.length){placeholdersArr.forEach(function(input,i){var placeholderEl=document.createElement('div');placeholderEl.className="placeholders hidden";placeholderEl.innerText=input.placeholder;$('.email-form').append(placeholderEl);});var originalPH=placeholdersFirst.placeholder;setInterval(function(){if(isTranslated()){updatePlaceholders();originalPH=placeholdersFirst.placeholder;}},500);var placeholdersDiv=document.querySelectorAll('.placeholders');var isTranslated=function isTranslated(){return originalPH!==_toConsumableArray(placeholdersDiv)[0].innerText;};var updatePlaceholders=function updatePlaceholders(){return placeholdersArr.forEach(function(item,i){item.placeholder=_toConsumableArray(placeholdersDiv)[i].innerText;originalPH=placeholdersFirst.placeholder;});};}

"use strict";grecaptcha.ready(function(){grecaptcha.execute('6LfQDnkUAAAAANwoRRk3zw8v1Vnr2vSkO_okRKr1',{action:'verifyToken'}).then(function(token){fetch("https://us-central1-bitnag-web-dev.cloudfunctions.net/recaptcha.php?action=verifyToken&token=".concat(token)).then(function(response){return response.json();}).then(function(success){return console.log(success);}).catch(function(error){return console.log(error);});});});

"use strict";function _toConsumableArray(arr){return _arrayWithoutHoles(arr)||_iterableToArray(arr)||_nonIterableSpread();}
function _nonIterableSpread(){throw new TypeError("Invalid attempt to spread non-iterable instance");}
function _iterableToArray(iter){if(Symbol.iterator in Object(iter)||Object.prototype.toString.call(iter)==="[object Arguments]")return Array.from(iter);}
function _arrayWithoutHoles(arr){if(Array.isArray(arr)){for(var i=0,arr2=new Array(arr.length);i<arr.length;i++){arr2[i]=arr[i];}return arr2;}}
var shareLinks=document.querySelectorAll('.btn-share');var documentUrl=document.URL;var SHARELINKS={twitter:{key:'twitter-share',path:"https://twitter.com/share?url=".concat(documentUrl)},facebook:{key:'facebook-share',path:"https://www.facebook.com/sharer/sharer.php?u=".concat(documentUrl)},linkedin:{key:'linkedin-share',path:"https://www.linkedin.com/shareArticle?mini=true&url=".concat(documentUrl,")")}};if(shareLinks){_toConsumableArray(shareLinks).forEach(function(item){return item.addEventListener('click',function(e){e.preventDefault();var currItem=e.currentTarget;var currItemDataId=currItem.getAttribute('data-js');var shareWindow;if(currItemDataId===SHARELINKS.twitter.key){shareWindow=window.open(SHARELINKS.twitter.path,'twitter-popup','left=5, top=5, margin=20, height=350, width=600');}else if(currItemDataId===SHARELINKS.facebook.key){shareWindow=window.open(SHARELINKS.facebook.path,'facebook-popup','left=5, top=5, margin=20, height=350, width=600');}else if(currItemDataId===SHARELINKS.linkedin.key){shareWindow=window.open(SHARELINKS.linkedin.path,'linkedin-popup','left=5, top=5, margin=20, height=350, width=600');}else{shareWindow='';}
...
function _iterableToArray(iter){if(Symbol.iterator in Object(iter)||Object.prototype.toString.call(iter)==="[object Arguments]")return Array.from(iter);}
function _arrayWithoutHoles(arr){if(Array.isArray(arr)){for(var i=0,arr2=new Array(arr.length);i<arr.length;i++){arr2[i]=arr[i];}return arr2;}}
var btnTabItems=document.querySelectorAll('.btn-tab');var tabContentItems=document.querySelectorAll('.tab-content__item');if(btnTabItems){_toConsumableArray(btnTabItems).forEach(function(item){return item.addEventListener('click',function(e){e.preventDefault();var currItem=e.currentTarget;var currItemDataId=currItem.getAttribute('data-tab');_toConsumableArray(tabContentItems).forEach(function(item){item.classList.add('hidden');if(item.id===currItemDataId){item.classList.remove('hidden');}});_toConsumableArray(btnTabItems).forEach(function(item){return item.classList.remove('active');});currItem.classList.add('active');});});}